...
class _StartShoppingState extends State<StartShopping> {
  ...
  Widget build(BuildContext context) {
    ...
    try {
      timeToShow = orderInfoJson[0]['delivery_details'][0]['delivery_day'] +
          "    (" +
          orderInfoJson[0]['delivery_details'][0]['delivery_hours_from'] +
          ":" +
          orderInfoJson[0]['delivery_details'][0]['delivery_minutes_from'] +
          "-" +
          orderInfoJson[0]['delivery_details'][0]['delivery_hours_to'] +
          ":" +
          orderInfoJson[0]['delivery_details'][0]['delivery_minutes_to'] +
          ")";
      commentToShow =
          orderInfoJson[0]['delivery_details'][0]['delivery_comment'] != null
              ? orderInfoJson[0]['delivery_details'][0]['delivery_comment']
              : 'N/A';
    } catch (e) {
      timeToShow = "N/A";
      commentToShow = "N/A";
                                          ...
                                          child: SizedBox(
                                            ...
                                            child: Container(
                                              ...
                                              child: Image(
                                                ...
                                                fit: BoxFit.cover,
                                                image: AssetImage(
                                                  'assets/images/drawer_backround.jpg',
                                                ),
                                              ),
                                            ),
                                          ),
  ...
  List<Widget> createProduct() {
    ...
    productWidget.add(Center(child: Container(
    ...
    for (int i = 0; i < productListToShow.length; i++) {
      dynamic singleProductCategory = productListToShow[i];
      bool isAdded = false;
      //print(singleProductCategory[0]['category_name']);

      List singleCategoryItems = singleProductCategory[0]['items'];
      for (int itemIndex = 0;
          itemIndex < singleCategoryItems.length;
          itemIndex++) {
        if (!isAdded) {
        ...
        productWidget.add(new ProductCell(
            singleCategoryItems[itemIndex], i, itemIndex, "1", () {
          setState(() {
            print('here');
          });
        }));
...
for(int doneIndex = 0; doneIndex < doneAddedList.length; doneIndex++)
  ...
  Future<void> activeOrder() async {
    ...
    try {
      ...
      final response = await http.post(changeStatus,
          headers: {
  ...
  List productListToShow = new List();
  ...
  void getOrderInfo() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        productListToShow = responseJson[0]['products'] as List;
